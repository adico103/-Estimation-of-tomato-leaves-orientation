...
class UR5_KIN():
    ...
    def get_start_point(self,rot,tcp):
        T_temp = self.get_tcp_wanted_rot(rot, self.d_base, tcp, self.r_plant)
        p_temp = T_temp[:3, 3]
        rv_temp = Rotation.from_matrix(T_temp[:3, :3])
        rv_temp = rv_temp.as_rotvec()
        # self.start = np.vstack([self.p, np.hstack([p_temp[:], rv_temp[:]])])
        self.start = np.hstack([p_temp[:], rv_temp[:]])
        return self.start 
